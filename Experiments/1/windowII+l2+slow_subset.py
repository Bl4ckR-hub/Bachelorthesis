...
best_model_checkpoint = '/user/viktor.tevosyan/u17320/project/Codebase/windowII+l2+slow+subset/best.pth'
latest_model_checkpoint = '/user/viktor.tevosyan/u17320/project/Codebase/windowII+l2+slow+subset/last.pth'
training_results_dir = '/user/viktor.tevosyan/u17320/project/Codebase/windowII+l2+slow+subset/'
...
def main(rank, world_size):
    ...
    reconstructor = Reconstructor.LearnableFBP(filtering_module=filtering_module,
                                               backprojection_module=vanilla_backproj, post_processing_module=no_post).to(rank)
    reconstructor = DDP(reconstructor, device_ids=[rank])

    optimizer = torch.optim.Adam(reconstructor.module.filtering_module.window_model.parameters(), lr=1e-3)
    ...
    trainer = Trainer.Trainer(model=reconstructor, train_loader=train_loader, valid_loader=valid_loader,
                              ...
                              one_cycle_lr=False, early_stopper_patience=4, training_results_dir=training_results_dir)
    ...
    if rank == 0:
        ...
        torch.save(trainer.metrics, training_results_dir + 'metrics.pth')
